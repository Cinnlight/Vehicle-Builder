...
class Cli {
    ...
    inquirer
      ...
      .then((answers: any) => {
        if (answers.vehicleType === 'Car') {
          ...
          // start the cli if the user selects an invalid option
          this.startCli();
        }
  ...
  findVehicleToTow(truck: Truck): void {
    if (!truck) return; // Ensure the truck parameter is used
    inquirer
      .prompt([
        {
          type: 'list',
      ...
      .then((answers: any) => {
        ...
        if (answers.vehicleToTow.vin === truck.vin) {
          ...
          console.log(`${truck.make} ${truck.model} is towing ${answers.vehicleToTow.make} ${answers.vehicleToTow.model}`);
          this.performActions();
          return;
        }
